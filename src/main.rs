...
use nix::unistd::{close, execvpe, pipe, read, write, setgid, sethostname, setuid, Gid, Uid};
use std::env;
use std::ffi::CString;
use std::fs::File;
use std::io::Write;
...
fn main() {
    ...
    let child_func = Box::new(|| child_main(pipe_read_fd.as_fd(), pipe_write_fd.as_raw_fd(), &c_args));
